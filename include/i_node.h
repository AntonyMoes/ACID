...
class INode : public IObservable<T>, IUpdateObserver<IComponent> {
  public:
    INode();
    void initialize_components(const std::map<size_t, IComponent>& comps);  // пока что заполняться из энтити будет;
                                                                            // TODO(AntonyMoes): устроить дальнейшую ревизию + хендлинг невозможности построиться
